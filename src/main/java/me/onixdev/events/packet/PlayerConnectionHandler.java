        ...
        try {
            ...
            if (user != null) {
                ...
                if (event.getPacketType() == PacketType.Configuration.Client.CLIENT_SETTINGS) {
                    ...
                    if (user.connectionStage == PlayerConnectionStep.LOGIN_SUCCESS_ACK)
                        user.connectionStage = PlayerConnectionStep.CLIENT_SETTINGS;
                }
                if (event.getPacketType() == PacketType.Configuration.Client.SELECT_KNOWN_PACKS) {
                    ...
                    if (user.connectionStage == PlayerConnectionStep.CLIENT_SETTINGS)
                        user.connectionStage = PlayerConnectionStep.SELECT_KNOWN_PACKS;
                }
                if (event.getPacketType() == PacketType.Play.Client.PLAYER_LOADED) {
                    user.ServerconnectionStage = PlayerConnectionStep.PLAYER_LOADED;
                    if (user.connectionStage == PlayerConnectionStep.SELECT_KNOWN_PACKS)
                        user.connectionStage = PlayerConnectionStep.PLAYER_LOADED;
                }
                if (event.getPacketType() == PacketType.Play.Client.PLUGIN_MESSAGE) {
                    user.ServerconnectionStage = PlayerConnectionStep.PLUGIN_MESSAGE;
                    if (user.connectionStage == PlayerConnectionStep.PLAYER_LOADED)
                        user.connectionStage = PlayerConnectionStep.PLUGIN_MESSAGE;
                }
                if (event.getPacketType() == PacketType.Play.Client.TELEPORT_CONFIRM) {
                    ...
                    if (user.connectionStage == PlayerConnectionStep.PLUGIN_MESSAGE)
                        user.connectionStage = PlayerConnectionStep.TELEPORT_CONFIRM;
                }
                if (event.getPacketType() == PacketType.Play.Client.HELD_ITEM_CHANGE) {
                    user.ServerconnectionStage = PlayerConnectionStep.HELD_ITEM_CHANGE;
                    if (user.connectionStage == PlayerConnectionStep.TELEPORT_CONFIRM)
                        user.connectionStage = PlayerConnectionStep.HELD_ITEM_CHANGE;
                }
