            ...
            if (args[0].equals("start")) {
                ...
                sender.spigot().sendMessage(
                    buildClickableMessage(
                        "/onix setup theme box",
                        "<aqua>Нажмите чтобы выбрать тематику сервера бокс пвп"
                    )
                )
                sender.spigot().sendMessage(
                    buildClickableMessage(
                        "/onix setup theme anka",
                        "<aqua>Нажмите чтобы выбрать тематику сервера Анархия"
                    )
                )
                ...
                if (thema != null) {
                    if (thema == "box") {
                        MessageUtil.sendMessage(
                            sender,
                            MessageUtil.miniMessage("<aqua>Вы выбрали тему сервера бокс пвп ждите античит настраивает оптимальный конфиг")
                        )
                    }
                }
            }
